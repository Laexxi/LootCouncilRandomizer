    ...
    function module:InitiateStatisticsSync()
        ...
        -- Speichere die Daten in einer Tabelle, um später darauf zugreifen zu können
        self.pendingSyncs = self.pendingSyncs or {}
        local syncKey = self:GetSyncKey(targetPlayer or syncTo)
    ...
    function module:HandleSyncAck(sender, message)
        local dataType = message.dataType
        debug:DebugPrint("Sync", "Received SyncAck from " .. sender .. " for data type " .. dataType)
        ...
        -- Verwende den synchronisierten Schlüssel
        local syncKey = self:GetSyncKey(sender)
        local syncInfo = self.pendingSyncs[syncKey]
    
        if not syncInfo or syncInfo.dataType ~= dataType then
            debug:DebugPrint("Sync", "No pending sync data for " .. sender .. " with type " .. dataType)
            return
        end
        ...
        if serializedData then
            self:SendCommMessage(SYNC_PREFIX, serializedData, "WHISPER", sender)
    ...
    function module:GetSyncKey(sender)
        local syncTo = LootCouncilRandomizer.db.profile.settings.syncTo
        if syncTo == "player" then
            return sender
        elseif syncTo == "guild" then
